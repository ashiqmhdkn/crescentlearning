...
import { Menu, Tag, HelpCircle, FileText, Info, ChevronDown, Building2,MessageSquareText, Images, ScrollText, Home } from "lucide-react"
import {
  NavigationMenu,
  ...
  const services = [
    {
      ...
      label: "CHTC",
      icon: Building2,
      description: "tuition centre for classes 8,9,10",
    },
    // {
    ...
    <header className="sticky top-0 z-50 p-4" role="banner">
      <div className="container mx-auto max-w-4xl">
        <div className="flex h-14 items-center justify-between px-6 liquid-glass-header rounded-full">
          {/* Brand Logo */}
          <Link href="/" className="flex items-center gap-1.5" aria-label="Crescent Learning Home">
            <Image
              ...
              className="h-8 w-8"
            />
            <span className="font-semibold tracking-wide text-white">Crescent Learning</span>
          </Link>

          {/* Desktop Nav with Services Dropdown */}
          <nav className="hidden items-center gap-6 text-sm text-white/90 md:flex" aria-label="Main navigation">
            <NavigationMenu>
              <NavigationMenuList>
                <NavigationMenuItem>
                  <NavigationMenuTrigger
              ...
              <SheetContent side="right" className="liquid-glass border-gray-800 p-0 w-64 flex flex-col">
                ...
                <div className="flex items-center gap-1.5 px-4 py-4 border-b border-gray-800">
                  <Image
                    ...
                    className="h-6 w-6"
                  />
                  <span className="font-semibold tracking-wide text-white text-lg">Crescent Learning</span>
                </div>

                {/* Nav Links */}
                <nav className="flex flex-col gap-1 mt-2 text-gray-200" aria-label="Mobile navigation">
